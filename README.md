...
run it will — you lose every variable and there is no way back except starting
over. Notebooks are the wrong tool for stages that take tens of minutes.

`run_all.py` is the whole pipeline as a resumable script. Every stage writes its
result to disk and is skipped if that result exists, so a crash costs you the
stage that was running and nothing before it.

```bash
# detached: survives closing the browser, logging out, losing wifi
nohup python run_all.py --clips clips/ --work work \
    --mask masks/left_front.json > work/log.txt 2>&1 &

tail -f work/log.txt        # watch progress; Ctrl-C only stops watching
```

Or under `tmux` on the VM, which also survives a closed tab:

```bash
tmux new -s fb
python run_all.py --clips clips/ --work work
# Ctrl-B then D detaches;  tmux attach -t fb  comes back
```

Stages: `detect` → `tiles` → `train` → `eval`. Run one with `--stage tiles`,
redo one with `--force`. Between `detect` and `tiles`, review the contact sheets
and list the non-firebrand track IDs in `work/clips/<clip>/reject.txt`.

**Use the notebook for looking, the script for producing.** Every stage leaves
files the notebook loads in seconds, so plots and diagnostics stay interactive
without holding an hour of compute in a kernel that might drop.
